...
void classificarAlunos (struct Aluno alunos[], int howManyStudents) {
	for (int position = 1; position <= howManyStudents; position++) {
		int thisTurnBestGuy;
...
void test1 () {
	...
	//	This line is giving some erros. media_geral from function and from literal are not being the same
